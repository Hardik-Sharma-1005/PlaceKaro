import { calculatePISFromFirebase } from "./service";
...
async function run(): Promise<void> {
  console.log("\n========================================");
  console.log("PlaceKaro Firebase PIS Integration Test");
  console.log("========================================\n");

  for (const studentId of TEST_STUDENTS) {
    try {
      const result = await calculatePISFromFirebase(
        studentId,
        JOB_ID
      );
...
run().catch((error) => {
  console.error(
    "Firebase PIS integration test failed:",
    error instanceof Error
      ? error.message
      : String(error)
  );
