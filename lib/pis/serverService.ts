import type {
  Achievement,
  Certification,
  Evidence,
  Internship,
  JobRequirements,
  PISConfiguration,
  Project,
  Skill,
  StudentProfile,
} from "../../types/database";

import { adminDatabase } from "../seed/firebaseAdmin";

import { calculatePIS } from "./engine";
import type { PISInput, PISResult } from "./types";

async function getAdminData<T>(
  path: string
): Promise<T | null> {
  const snapshot = await adminDatabase
    .ref(path)
    .once("value");

  if (!snapshot.exists()) {
    return null;
  }

  return snapshot.val() as T;
}

async function getAdminDataByChild<T>(
  collectionPath: string,
  childPath: string,
  value: string | number | boolean
): Promise<T[]> {
  const snapshot = await adminDatabase
    .ref(collectionPath)
    .orderByChild(childPath)
    .equalTo(value)
    .once("value");

  if (!snapshot.exists()) {
    return [];
  }

  const data = snapshot.val() as Record<
    string,
    T
  >;

  return Object.values(data);
}

async function getStudentProfile(
  studentId: string
): Promise<StudentProfile> {
  const profile =
    await getAdminData<StudentProfile>(
      `studentProfiles/${studentId}`
    );

  if (!profile) {
    throw new Error(
      `Student profile not found for ${studentId}.`
    );
  }

  return profile;
}

async function getJobRequirements(
  jobId: string
): Promise<JobRequirements> {
  const requirements =
    await getAdminData<JobRequirements>(
      `jobRequirements/${jobId}`
    );

  if (!requirements) {
    throw new Error(
      `Job requirements not found for ${jobId}.`
    );
  }

  return requirements;
}

async function getPISConfiguration(
  jobId: string
): Promise<PISConfiguration> {
  const configuration =
    await getAdminData<PISConfiguration>(
      `pisConfigurations/${jobId}`
    );

  if (!configuration) {
    throw new Error(
      `PIS configuration not found for ${jobId}.`
    );
  }

  if (configuration.confirmed !== true) {
    throw new Error(
      `PIS configuration for ${jobId} has not been confirmed.`
    );
  }

  return configuration;
}

async function getCandidateData(
  studentId: string
): Promise<{
  skills: Skill[];
  projects: Project[];
  internships: Internship[];
  certifications: Certification[];
  achievements: Achievement[];
  evidence: Evidence[];
}> {
  const [
    skills,
    projects,
    internships,
    certifications,
    achievements,
    evidence,
  ] = await Promise.all([
    getAdminDataByChild<Skill>(
      "skills",
      "studentId",
      studentId
    ),

    getAdminDataByChild<Project>(
      "projects",
      "studentId",
      studentId
    ),

    getAdminDataByChild<Internship>(
      "internships",
      "studentId",
      studentId
    ),

    getAdminDataByChild<Certification>(
      "certifications",
      "studentId",
      studentId
    ),

    getAdminDataByChild<Achievement>(
      "achievements",
      "studentId",
      studentId
    ),

    getAdminDataByChild<Evidence>(
      "evidence",
      "studentId",
      studentId
    ),
  ]);

  return {
    skills,
    projects,
    internships,
    certifications,
    achievements,
    evidence,
  };
}

export async function buildServerPISInput(
  studentId: string,
  jobId: string
): Promise<PISInput> {
  if (!studentId) {
    throw new Error(
      "studentId is required."
    );
  }

  if (!jobId) {
    throw new Error(
      "jobId is required."
    );
  }

  const [
    profile,
    requirements,
    configuration,
    candidateData,
  ] = await Promise.all([
    getStudentProfile(studentId),
    getJobRequirements(jobId),
    getPISConfiguration(jobId),
    getCandidateData(studentId),
  ]);

  return {
    candidate: {
      profile,
      skills: candidateData.skills,
      projects: candidateData.projects,
      internships: candidateData.internships,
      certifications:
        candidateData.certifications,
      achievements:
        candidateData.achievements,
      evidence: candidateData.evidence,
    },

    job: {
      requirements,
    },

    configuration,
  };
}

export async function calculatePISFromAdminFirebase(
  studentId: string,
  jobId: string
): Promise<PISResult> {
  const input =
    await buildServerPISInput(
      studentId,
      jobId
    );

  return calculatePIS(input);
}
