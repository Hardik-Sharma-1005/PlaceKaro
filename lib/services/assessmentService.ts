...
export const assessmentService = {
  ...
  async getAssessmentByJobId(jobId: string): Promise<Assessment | null> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch assessment (permission denied or network error):", error);
      return null;
    }
  },
  ...
  async createAssessment(assessmentData: Omit<Assessment, "id" | "createdAt">): Promise<string> {
    try {
      ...
      throw error; // Write errors should still surface
    }
  },
  ...
  async getQuestionsByAssessmentId(assessmentId: string): Promise<AssessmentQuestion[]> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch questions (permission denied or network error):", error);
      return [];
    }
  },
