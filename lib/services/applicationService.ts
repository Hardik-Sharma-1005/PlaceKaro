...
export const applicationService = {
  ...
  async getApplication(studentId: string, jobId: string): Promise<Application | null> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch application (permission denied):", error);
      return null;
    }
  },
  ...
  async getAssessmentResult(studentId: string, assessmentId: string): Promise<AssessmentResult | null> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch assessment result (permission denied):", error);
      return null;
    }
  },
  ...
  async getApplicationsForJob(jobId: string): Promise<Application[]> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch applications for job (permission denied):", error);
      return [];
    }
  },
  ...
  async getApplicationsForStudent(studentId: string): Promise<Application[]> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch applications for student (permission denied):", error);
      return [];
    }
  },
  ...
  async getAssessmentResultByStudentAndJob(studentId: string, jobId: string): Promise<AssessmentResult | null> {
    try {
      ...
      // Silently handled: permission denied // console.debug("Could not fetch assessment result by job (permission denied):", error);
      return null;
    }
  }
};
